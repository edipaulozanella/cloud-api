...
function NovoCliente() {
  var cf = {
    ...
    //   database: database,
    //   port: 5432,
    //   host: "aa1qfomhjjdw67b.ctd4wbbp2rak.sa-east-1.rds.amazonaws.com",
    //   ssl: true
    // });
    client.connect();
    try {
      exe("SET SESSION timezone TO 'Brazil/East'", function () {
        console.log("SET SESSION timezone TO 'Brazil/East'");
      });
    } catch (e) {
    }
...
function makeWhereSql(where) {
  ...
  for (var i = 0; i < lista.length; i++) {
    ...
    if (value.$gt) {
      ...
      whereSql += " \"" + nome + "\" IN " + JSON.stringify(value.$in).replace("{", "[").replace("}", "]");
    } else if (value.$ne) {
      ...
      whereSql += " \"" + nome + "\" NO IN " + JSON.stringify(value.$nin).replace("{", "[").replace("}", "]");
...
function processarAlteracaoSql(obj, entidade, retorno) {
  ...
  function getSql(obj, entidade) {
    ...
    try {
      switch (entidade) {
        default: sql = MontarInsert(obj, data, entidade);
        break;
      }
    } catch (e) {
      console.log(e);
...
exports.updateColunas = function(data, entidade, colunas, retorno) {
  if (!entidade) {
    entidade = "base";
  }
...
function corrigirErrosBanco(entidade, obj, callback) {
  var criar = "CREATE TABLE IF NOT EXISTS " + entidade + " ( " + " \"objectId\" character varying ,  " + " \"createdAt\"   timestamp with time zone DEFAULT now() ,  " + " \"updatedAt\"  timestamp with time zone DEFAULT now() ,  "
    // + ' "createdAt"  character varying ,  '
    // + ' "updatedAt"  character varying ,  '
    + " status int DEFAULT 1 ,  " + " _id serial  " + "  ); ";
  exe(criar, function() {
